...
//					score -= 30;			// -30 om draget leder till att han blir kung
	...
	if(((b.black & (~b.kings) & 0x0F000000)<<4) & (~allPieces)) score += 40;	// +40 för svart vänsterdrag som ger kung
	if(((b.black & (~b.kings) &  0x0F000000)<<5) & (~allPieces)) score += 40;	// +40 för svart högerdrag som ger kung

	if(((b.white & (~b.kings) & 0x000000F0)>>4) & (~allPieces)) score -= 40; // -40 för vitt vänsterdrag som ger kung
	if(((b.white & (~b.kings) & 0x000000F0)>>5) & (~allPieces)) score -= 40; // -40 för vitt vänsterdrag som ger kung
	...
	score += (countBits(b.black & 0x0000000F)<<3);	// upp till +32 för stark svart "back rating"
	score -= (countBits(b.white & 0xF0000000)<<3);	// upp till -32 för stark vit "back rating"
